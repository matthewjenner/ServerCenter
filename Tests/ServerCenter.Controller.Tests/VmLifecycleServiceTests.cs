...
public sealed class VmLifecycleServiceTests : IAsyncLifetime
{
    ...
    public async Task Start_runs_libvirt_and_persists_a_succeeded_vm_start_job()
    {
        ...
        FakeLibvirtHost libvirt = new FakeLibvirtHost();
        libvirt.Seed("cs2-ffa", "uuid-1", DomainState.ShutOff);

        VmDispatchResult result = await Service(libvirt).DispatchAsync("cs2-node", VmAction.Start, ct);
    ...
    {
        VmDispatchResult result = await Service(new FakeLibvirtHost())
    ...
    public async Task A_libvirt_failure_persists_a_failed_job()
    {
        ...
        VmDispatchResult result = await Service(new NullLibvirtHost()).DispatchAsync("cs2-node", VmAction.Stop, ct);
