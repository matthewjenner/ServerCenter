...
public sealed class JobRepositoryTests : IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        AgentNodeRepository agents = new AgentNodeRepository(_db.Database);
    ...
    {
        CancellationToken ct = TestContext.Current.CancellationToken;
        Job job = NewJob("j1", requeueable: true);
        await _jobs.InsertAsync(job, ct);

        Job? got = await _jobs.GetAsync("j1", ct);
    ...
    public async Task GetOpenJobsForAgent_excludes_terminal_jobs()
    {
        ...
        IReadOnlyList<Job> open = await _jobs.GetOpenJobsForAgentAsync("agent-1", ct);
    ...
    public async Task ListRecentJobs_returns_newest_first()
    {
        ...
        IReadOnlyList<Job> recent = await _jobs.ListRecentJobsAsync(10, ct);
