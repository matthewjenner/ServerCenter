...
// Turns "install / configure this server instance" into a persisted job. It resolves the instance +
// its pinned descriptor controller-side (the agent never sees the descriptor), builds the concrete
// job params, and dispatches. Install pulls the SteamCMD app from the descriptor; config-apply
// resolves the instance params and ships each config file's template text with the job.
